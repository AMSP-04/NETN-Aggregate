...
|SourceUnit|Aggregate from which this aggregate was spawned.|
...
|UnitSupplies|The type and quantities of supplies available (on hand) to the unit.|
|HigherHeadquarters|A pointer to the aggregate's superior unit or headquarters. The highest level unit or headquarters on each side will publish its own UniqueID as its HigherHeadquarters value.|
|CombatValue|A summary value (in percent) of unit effectiveness based on level of training, leadership, moral, personnel and equipment operational status, etc.|
...
|Symbol|The APP6A/MILSTD 2525B/MILSTD 2525C code for the aggregate.|
...
|Echelon|The level of command of the aggregate.|
...
|EntityList|Provides data on entities comprising the aggregate.|
|Status|If an instance shall be taken into account by federates.|
|UniqueID|The unique identifier of the object.|
|EmbeddedUnitList|The list of objects carried by this aggregate.|
